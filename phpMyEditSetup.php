    ...
    foreach ([
        ...
        if (@file_exists($file) && @is_readable($file)) {
            ...
            if (strpos($str, ' ') === false && strlen($str) < 10) {
                $ret_ar[$type] = $str;
            } elseif (preg_match('|\$' . 'Platon:\s+\S+,v\s+(\d+.\d+)\s+|', $str, $matches)) {
...
END;
    ...
    foreach ([
        ...
        'current' => 'generating setup script:'] as $type => $desc) {
...
END;
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        } elseif ($key == 'PRI') {       // ok if auto increment
