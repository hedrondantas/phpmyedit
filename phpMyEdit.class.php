...
class phpMyEdit
{
    ...
    */
	...
	{ /* covers also filtering page */
		return ! $this->change_operation()
	...
	public function processed($k)
	{
		...
		return ($this->saveadd == $this->labels['Save'] && stristr($options, 'A'))
	...
	public function displayed($k)
	{
		...
		return ($this->add_operation() && stristr($options, 'A'))
    ...
    */
	...
	public function form_begin()
	{
		...
		if (
			$this->add_operation() || $this->change_operation() || $this->copy_operation()
...
{';
	...
	document.' . $this->cgi['prefix']['sys'] . 'form.' . $this->cgi['prefix']['sys'] . 'cur_tab.value = tab_name;
		...
		if ($this->add_operation() || $this->change_operation() || $this->copy_operation()) {
			...
			for ($k = 0; $k < $this->num_fds; $k++) {
				if (
					$this->displayed[$k] && ! $this->readonly($k) && ! $this->hidden($k)
...
function ' . $this->js['prefix'] . 'filter_handler(theForm, theEvent)
{
	 ...
	 * @param	js		string to be in the <select >, ususally onchange='..';
	 */
	public function htmlRadioCheck(
		$name,
		$css,
		$kv_array,
		$selected = null, /* ...) {{{ */
