...
{
    public function phpMyEdit_report($opts) /* {{{ */
    {
        $opts['options'] = 'L';
        $execute = 1;
    ...
    public function get_cgi_cookie_var($name, $default_value = null) /* {{{ */
    {
    ...
    public function display_list_table_buttons($total_recs, $position) /* {{{ */
    ...
    public function display_report_selection_buttons($position) /* {{{ */
    {
        echo '<table class="',$this->getCSSclass('navigation', $position),'">',"\n";
        echo '<tr class="',$this->getCSSclass('navigation', $position),'">',"\n";
        echo '<td class="',$this->getCSSclass('buttons', $position),'">',"\n";
    ...
    public function get_select_fields_link() /* {{{ */
    {
    ...
    public function execute() /* {{{ */
    {
        ...
        foreach (array_merge(['@inc'], array_keys($table_cols)) as $col) {
    ...
    public function execute_report_screen($table_cols) /* {{{ */
    {
        ...
        foreach ($table_cols as $key => $val) {
            ...
            if ($this->fdd[$key]['select'] == 'D' || $this->fdd[$key]['select'] == 'M') {
                ...
                $selected   = $value;
                $value      = $this->set_values($key, ['*' => '*'], null, $from_table);
